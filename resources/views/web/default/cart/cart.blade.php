                        ...
                        <div class="col-6 col-lg-2 d-flex flex-md-column align-items-center justify-content-center">
                            ...
                            @if(!empty($cart->webinar_id))
                                @if($cart->webinar->getDiscount($cart->ticket))
                                    <span class="text-gray text-decoration-line-through mx-10 mx-md-0">{{ handlePrice($cart->webinar->price, true, true) }}</span>
                                    <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cart->webinar->price - $cart->webinar->getDiscount($cart->ticket), true, true) }}</span>
                                @else
                                    <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cart->webinar->price, true, true) }}</span>
                                @endif

                            @elseif(!empty($cart->reserve_meeting_id))
                                <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cart->reserveMeeting->paid_amount, true, true) }}</span>
                            @endif
                        </div>
